...
public class BroadcastOperator extends Operator {
    ...
    protected void execute() {
        ...
        var broadcastParams = OperatorParameterUtils.getColumns("broadcast", parameters);
    ...
    private CodeBlock getCodeBlock(Map<String, String> primaryParams,
        ...
        var descriptorNameVariable = NameHelper.generateVariableName(DESCRIPTOR_NAME);
        var connectDS = NameHelper.generateVariableName("connectDs");
        var resultDS = NameHelper.generateVariableName("resultDS");
        var primaryTableName = getTableNameByInputPort(primaryInput);
        var primaryTableDs = NameHelper.generateVariableName(primaryTableName, "Ds");
        var broadTableName = getTableNameByInputPort(broadcastInput);
        var broadTableNameDs = NameHelper.generateVariableName(broadTableName, "Ds");
        ...
        var singleOutputStreamOperatorTupleN =
            ParameterizedTypeName.get(Specifications.SINGLE_OUTPUT_STREAM_OPERATOR, outputTupleNType);

        var broadcastProcessFunctionRow2TupleN = ParameterizedTypeName
        ...
        var context = broadcastProcessFunctionRow2TupleN.nestedClass("Context");

        var outputCodeBlock = createOutput(primaryParams, broadcastParams, typeNClass);
        ...
        var columnsNames = OperatorParameterUtils.generateColumnNames(outputParams);
    ...
    private TypeSpec getBroadcastProcessFunction(String descriptorNameVariable,
                                                 ...
                                                 CodeBlock codeBlock) {
        final var broadcastState = "broadcastState";
    ...
    private CodeBlock createOutput(Map<String, String> primaryColumns,
        ...
        for (var pc : primaryColumns.entrySet()) {
            if (sentinel) {
                cb.add(", ");
            }

            sentinel = true;
            var cl = OperatorParameterUtils.getClassNameByName(pc.getValue());
            cb.add(GET_FIELD, cl, VALUE, pc.getKey());
        }

        for (var pc : broadcastColumns.entrySet()) {
            var cl = OperatorParameterUtils.getClassNameByName(pc.getValue());
            cb.add(", ");
    ...
    private String getTableNameByInputPort(InputPortObject<TableInfo> inputPortObject) {
        String pseudoName = "";
        var primaryPseudoData = inputPortObject.getOutputPseudoData();
