...
public abstract class AbstractSqlTable extends Operator {
    ...
    /** Source校验内容： WATERMARK中的字段是否为事件字段 Sink校验内容： WATERMARK中的字段是否为事件字段 入参数量和表字段数量不匹配 类型不匹配 */
    @Override
        ...
        if (watermark != null) {
            ...
            Map<String, String> type =
                    columns.stream()
                            .filter(
                                    item ->
                                            item.get(NAME).equals(watermark.get(COLUMN))
                                                    && item.get(TYPE).contains("TIME"))
                            .findFirst()
                            .orElse(null);
            if (type == null) {
    ...
    public void processLogic(
            OutputPortObject<TableInfo> outputPortObject, Map<String, Object> dataModel) {
    ...
    public void connectToSink(
            String input_0, Map<String, Object> dataModel, List<Map<String, Object>> input) {
    ...
    protected Map<String, Object> getDataModel() {
        ...
        if (watermark != null
                && watermark.get(COLUMN) != null
                && !watermark.get(COLUMN).equals("")) {
        ...
        Map<String, List<Map<String, Object>>> outPut =
                (Map<String, List<Map<String, Object>>>) parameterLists.get(0).get(OUTPUT);
