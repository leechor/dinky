...
public class FieldFunction {
    ...
    /**
     * 解析字段处理方法配置, 生成{@link FieldFunction}定义 所有字段的通用方法，可仅指定parameters，表示该字段不做任何处理
     *
     * @param fos 字段配置信息
     * @return 方法定义
     */
    public static FieldFunction processFieldConfigure(
            String tableName,
            Map<String, Object> fos,
            boolean flag,
        ...
        if (columns != null && !columns.isEmpty()) {
            pretreatment =
                    pretreatment(tableName, columns, flag, inputColumns, new StringBuffer(), "")
                            .toString();
        }
    ...
    public static StringBuffer pretreatment(
            String tableName,
            List<Map<String, Object>> recursion,
            boolean flag,
            List<Map<String, Object>> inputColumns,
            StringBuffer str,
            String last) {
    ...
    public static void recursion(
        ...
        if (!inputName.get(nameOrFunc).toString().equals("[]")) {
            ...
            @SuppressWarnings("unchecked")
            List<Map<String, Object>> recursionFunc =
                    ((List<Map<String, Object>>) inputName.get(nameOrFunc));
            pretreatment(
                    tableName, recursionFunc, flag, inputColumns, str, now.equals("") ? last : now);
        ...
        for (Map<String, Object> fos : funcs) {
            ...
            fo.setOutType(
                    typeInference(
                            inputColumn,
                            fo,
                            fos.get(TYPE) == null ? null : fos.get(TYPE).toString()));
