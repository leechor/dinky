...
public class CustomerOperator extends Operator {
    ...
    public Optional<OperatorFeature> getOperatorFeature() {
        OperatorFeature operatorFeature = OperatorFeature.builder().icon("icon-xinjianku").build();
    ...
    protected Map<String, Object> formatOperatorParameter() {
        ...
        if (getInputPorts() != null && !getInputPorts().isEmpty()) {
            ...
            for (String s : collect) {
                ...
                String tableName =
                        ((InputPortObject<TableInfo>) getInputPorts().get(s))
                                .getOutputPseudoData()
                                .getName();
    ...
    protected void execute(Map<String, Object> result) {
        ...
        this.getSceneCode()
                .getGenerateResult()
                .generate((String) result.get("statementBody")); // 本算子输入的sql不用转换
