    ...
    <>
      {statusCounts?.CREATED > 0 ? (
        <Tooltip title="CREATED">
          <Tag color="#666">{statusCounts.CREATED}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.INITIALIZING > 0 ? (
        <Tooltip title="INITIALIZING">
          <Tag color="#666">{statusCounts.INITIALIZING}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.DEPLOYING > 0 ? (
        <Tooltip title="DEPLOYING">
          <Tag color="#666">{statusCounts.DEPLOYING}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.RUNNING > 0 ? (
        <Tooltip title="RUNNING">
          <Tag color="#44b549">{statusCounts.RUNNING}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.FAILED > 0 ? (
        <Tooltip title="FAILED">
          <Tag color="#ff4d4f">{statusCounts.FAILED}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.FINISHED > 0 ? (
        <Tooltip title="FINISHED">
          <Tag color="#108ee9">{statusCounts.FINISHED}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.RECONCILING > 0 ? (
        <Tooltip title="RECONCILING">
          <Tag color="#666">{statusCounts.RECONCILING}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.SCHEDULED > 0 ? (
        <Tooltip title="SCHEDULED">
          <Tag color="#666">{statusCounts.SCHEDULED}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.CANCELING > 0 ? (
        <Tooltip title="CANCELING">
          <Tag color="#feb72b">{statusCounts.CANCELING}</Tag>
        </Tooltip>
      ) : undefined}
      {statusCounts?.CANCELED > 0 ? (
        <Tooltip title="CANCELED">
          <Tag color="#db970f">{statusCounts.CANCELED}</Tag>
        </Tooltip>
      ) : undefined}
    </>
  );
