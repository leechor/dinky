...
export class MyAutoCompleteEditor extends StringEditor {
  ...
  afterInputReady() {
    ...
    let options = this.expandCallbacks(
      'autoinput',
      extend(
        {},
        {},
        this.defaults.options.autoinput || {},
        this.options.autoinput || {},
        this.options.schema.autoinput || {},
      ),
    );
    ...
    const autoCompleteJS = new autoComplete(
      {
        ...
        data: {
          ...
          filter: (list) => {
            // Filter duplicates
            return Array.from(new Set(list.map((value) => value.match))).map((item) => {
