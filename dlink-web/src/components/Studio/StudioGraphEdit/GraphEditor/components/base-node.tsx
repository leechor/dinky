...
const BaseNode = (props: { nodeType: NodeType; iconPath: string; name: string }) => {
      ...
      <>
        ...
        {portInformation && (
          <div className={styles['error-msg']}>
            ...
            <div className={styles['error-msg-body']}>
              {portInformation &&
                Object.keys(portInformation).map((port) => {
                  return (
                    <div>
                      <div>
                        {' '}
                      ...
                      <div>
                        {portInformation[port] &&
                          portInformation[port].map((info: string, key: number) => {
                            return (
                              <div key={key} style={{ textIndent: '1em' }}>
                                <span>{key + 1}: </span>
                                <span>{info}</span>
                              </div>
                            );
  ...
  return (
    <>
      {!node.prop().isStencil && (
        <div className={styles['custom-calcu-node-head']}>
          <div style={{ whiteSpace: 'nowrap', textAlign: 'center' }}>
            {node.prop().name ? node.prop().name : name}
          </div>
        </div>
      )}
      <div className={`${styles['custom-calcu-node']} ${isShowMenu ? 'custom-calcu-node' : ''}`}>
        {node && node.prop().isStencil && (
          <Tooltip title={name ? name : node.prop().name}>
            <div className={styles['custom-calcu-node-label']}>
              {name ? name : node.prop().name}
            </div>
          </Tooltip>
        )}
        ...
        {node.prop().isError && (
          <div className={styles['custom-calcu-node-waring']} onClick={handleWarning}>
