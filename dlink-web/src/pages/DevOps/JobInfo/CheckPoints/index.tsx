...
const CheckPoints = (props: any) => {
          ...
          <Descriptions bordered size="small" column={1}>
            ...
            <Descriptions.Item label="Latest Failed CheckPoint">
              {latest.failed === null ? (
                ...
                <>
                  ...
                  <Tag color="red" title={'Latest Failed CheckPoint'}>
                    {'Fail Time： ' +
                      moment(JsonParseObject(latest.failed).failure_timestamp).format(
                        'YYYY-MM-DD HH:mm:ss',
                      )}
                  </Tag>
                  <Tag color="red" title={'Latest Failed CheckPoint'}>
            ...
            <Descriptions.Item label="Latest Restored">
              <Tag color="cyan" title={'Latest Restored'}>
                {latest.restored === null ? 'None' : JsonParseObject(latest.restored).external_path}
              </Tag>
            </Descriptions.Item>
  ...
  const getSummary = (checkpoints: any) => {
    let end_to_end_duration = JsonParseObject(
      JsonParseObject(checkpoints.summary),
    ).end_to_end_duration;
    ...
    let alignment_buffered = JsonParseObject(
      JsonParseObject(checkpoints.summary),
    ).alignment_buffered;
  ...
  function recoveryCheckPoint(row: CheckPointsDetailInfo) {
    Modal.confirm({
      ...
      onOk: async () => {
        const res = selectSavePointRestartTask(
          job?.instance?.taskId,
          job?.instance?.step == JOB_LIFE_CYCLE.ONLINE,
          row.external_path,
        );
        res.then((result) => {
  ...
  const getHistory = (checkpoints: any) => {
    ...
    const columns: ProColumns<CheckPointsDetailInfo>[] = [
      ...
      },
      {
        ...
        copyable: true,
        render: (dom, entity) => {
          return entity.end_to_end_duration === null
            ? 'None'
            : parseMilliSecondStr(entity.end_to_end_duration);
        },
      },
      {
          ...
          return (
            <>
              {entity.status === 'COMPLETED' ? (
                <Button onClick={() => recoveryCheckPoint(entity)}>此处恢复</Button>
              ) : undefined}
            </>
          );
        },
          ...
          <Descriptions bordered size="small" column={1}>
            ...
            <Descriptions.Item label="Checkpoint Storage">
              <Tag color="blue" title={'Checkpoint Storage'}>
                {checkpointsConfigInfo.checkpoint_storage
                  ? checkpointsConfigInfo.checkpoint_storage
                  : 'Disabled'}
              </Tag>
            </Descriptions.Item>
            ...
            <Descriptions.Item label="State Backend">
              <Tag color="blue" title={'State Backend'}>
                {checkpointsConfigInfo.state_backend
                  ? checkpointsConfigInfo.state_backend
                  : 'Disabled'}
              </Tag>
            </Descriptions.Item>
            ...
            {JsonParseObject(checkpointsConfigInfo.externalization).enabled && (
              <Descriptions.Item label="Delete On Cancellation">
                <Tag color="blue" title={'Delete On Cancellation'}>
                  {JsonParseObject(checkpointsConfigInfo.externalization).delete_on_cancellation
                    ? 'Enabled'
                    : 'Disabled'}
                </Tag>
              </Descriptions.Item>
