...
const ClusterConfigurationForm: React.FC<ClusterConfigurationFormProps> = (props) => {
  ...
  const [hadoopConfigPath, setHadoopConfigPath] = useState<string>(
    getConfigFormValues(formVals)['hadoopConfigPath'],
  );
  const [flinkLibPath, setFlinkLibPath] = useState<string>(
    getConfigFormValues(formVals)['flinkLibPath'],
  );
  const [flinkConfigPath, setFlinkConfigPath] = useState<string>(
    getConfigFormValues(formVals)['flinkConfigPath'],
  );
  ...
  const buildConfig = (config: Config[], formValsPara: any) => {
    ...
    config.forEach((configItem) => {
      if (
        configItem.showOnSubmitType != undefined &&
        configItem.showOnSubmitType != formValsPara.type
      ) {
        //    pass
      } else {
        if (configItem.showType == 'input' || configItem.showType == undefined) {
          itemList.push(
            <Form.Item name={configItem.name} label={configItem.lable}>
  ...
  const renderFlinkKubernetesOperatorPage = (
    formValsPara: Partial<ClusterConfigurationTableListItem>,
  ) => {
    return (
      <>
        <Divider>{l('pages.rc.clusterConfig.k8sConfig')}</Divider>
        <Form.Item name="flinkVersion" label={l('pages.rc.clusterConfig.kubernets.version')}>
          <Select>
            <Option value="v1_13">
              v1_13 ({l('pages.rc.clusterConfig.kubernets.unsupportBatch')})
            </Option>
            <Option value="v1_14">
              v1_14 ({l('pages.rc.clusterConfig.kubernets.unsupportBatch')})
            </Option>
  ...
  const renderFlinkKubernetesNativePage = (
    formValsPara: Partial<ClusterConfigurationTableListItem>,
  ) => {
    return (
      <>
        ...
        <Form.Item
          ...
          rules={[
            { required: true, message: l('pages.rc.clusterConfig.flinkConfigPathPlaceholder') },
          ]}
          help={l('pages.rc.clusterConfig.flinkConfigPathHelp')}
        >
          <Input
            placeholder={l('pages.rc.clusterConfig.flinkConfigPathPlaceholder')}
            addonAfter={
    ...
    return (
      <>
        ...
        <Form.Item
          ...
          rules={[
            { required: true, message: l('pages.rc.clusterConfig.hadoopConfigPathPlaceholder') },
          ]}
          help={l('pages.rc.clusterConfig.hadoopConfigPathHelp')}
        >
          <Input
            placeholder={l('pages.rc.clusterConfig.hadoopConfigPath')}
            addonAfter={
              <Form.Item name="suffix" noStyle>
        ...
        <Divider orientation="left" plain>
          {l('pages.rc.clusterConfig.defineConfig.highPriority')}
        </Divider>
        ...
        >
          <Input
            placeholder={l('pages.rc.clusterConfig.libPathPlaceholder')}
            addonAfter={
              <Form.Item name="suffix" noStyle>
  ...
  const renderContent = (formValsPara: Partial<ClusterConfigurationTableListItem>) => {
    return (
      <>
        <Form.Item name="type" label={l('pages.rc.clusterConfig.type')}>
        ...
        {formValsPara.type == 'Kubernetes'
          ? renderFlinkKubernetesNativePage(formValsPara)
          : undefined}

        {formValsPara.type == 'KubernetesOperator'
          ? renderFlinkKubernetesOperatorPage(formValsPara)
          : undefined}
        ...
        <Form.Item
          ...
          help={
            l('pages.rc.clusterConfig.sqlSubmitJarPath') + ' eg: hdfs:///dlink/jar/dlink-app.jar'
          }
        >
