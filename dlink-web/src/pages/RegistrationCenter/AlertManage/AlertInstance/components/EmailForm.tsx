  ...
  const renderContent = (vals) => {
    return (
      <>
        ...
        >
          <Input.TextArea
            placeholder={l('pages.rc.alert.instance.receiversPleaseHolder')}
            allowClear
            autoSize={{ minRows: 1, maxRows: 5 }}
          />
        </Form.Item>
        <Form.Item name="receiverCcs" label={l('pages.rc.alert.instance.receiverCcs')}>
          <Input.TextArea
            placeholder={l('pages.rc.alert.instance.receiverCcsPleaseHolder')}
            allowClear
            autoSize={{ minRows: 1, maxRows: 5 }}
          />
        </Form.Item>
        ...
        <Form.Item name="enableSmtpAuth" label={l('pages.rc.alert.instance.enableSmtpAuth')}>
          <Switch
        ...
        {vals.enableSmtpAuth && (
          <>
            <Form.Item
              ...
              rules={[
                { required: true, message: l('pages.rc.alert.instance.emailUserPleaseHolder') },
              ]}
            >
            ...
            <Form.Item
              ...
              rules={[
                { required: true, message: l('pages.rc.alert.instance.emailPasswordPleaseHolder') },
              ]}
            >
              <Input.Password
                allowClear
                placeholder={l('pages.rc.alert.instance.emailPasswordPleaseHolder')}
              />
            </Form.Item>
          </>
        )}
        <Form.Item name="starttlsEnable" label={l('pages.rc.alert.instance.starttlsEnable')}>
          <Switch
            checkedChildren={l('button.enable')}
            unCheckedChildren={l('button.disable')}
        ...
        <Form.Item name="sslEnable" label={l('pages.rc.alert.instance.sslEnable')}>
          <Switch
            checkedChildren={l('button.enable')}
            unCheckedChildren={l('button.disable')}
        ...
        {vals.sslEnable && (
          <Form.Item
            ...
            rules={[
              { required: true, message: l('pages.rc.alert.instance.smtpSslTrustPleaseHolder') },
            ]}
          >
        ...
        {(vals.msgtype === 'attachment' || vals.msgtype === 'table attachment') && (
          <>
            <Form.Item name="xls.file.path" label={l('pages.rc.alert.instance.xls.file.path')}>
              <Input.TextArea
                placeholder={l('pages.rc.alert.instance.xls.file.pathPleaseHolder')}
                allowClear
                autoSize={{ minRows: 1, maxRows: 5 }}
              />
            </Form.Item>
