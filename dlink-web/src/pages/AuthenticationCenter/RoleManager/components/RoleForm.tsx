...
const RoleForm: React.FC<TenantFormProps> = (props) => {
  ...
  const { onSubmit: handleSubmit, onCancel: handleModalVisible, modalVisible, nameSpaces } = props;
  ...
  const getNameSpaceOptions = () => {
    ...
    for (const item of nameSpaces) {
      const tag = (
        <>
          <Tag color="processing">{item.namespaceCode}</Tag>
          {/*{item.namespaceCode}*/}
        </>
      );
      itemList.push(
        <Option key={item.namespaceCode} value={item.id?.toString()} label={tag}>
          {tag}
        </Option>,
      );
    }
    ...
    return (
      <>
        ...
        <Form.Item name="note" label={l('global.table.note')}>
          <Input.TextArea
            placeholder={l('pages.role.EnterNote')}
            allowClear
