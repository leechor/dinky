...
import { getKeyWordByType } from '@/pages/DataCenter/MetaData/Console/function';
...
const Console = (props: any) => {
  ...
  const reloadCompletion = () => {
    provider.dispose();
    provider = monacoInstance.current.languages.registerCompletionItemProvider(
      currentDatabase.type.toLowerCase(),
      {
  ...
  const execSql = async () => {
    ...
    setLoading(true);
    let temp: { columns: ProColumns[]; rowData: {}[]; msg: string } = {
      rowData: [],
      columns: [],
      msg: '',
    };
    ...
    await execDatabaseSql(dbId, selectSql).then((result) => {
      ...
      temp.msg = l('pages.metadata.exec.result.msg', '', {
        time: data.time / 1000,
        rowCount: data.total,
      });
      if (data.total >= 500) {
